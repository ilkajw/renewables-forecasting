...
# ── Option 1 — Raw absolute values, capacity in kw ────────────────────────────────────────────
#
...
# ── Option 2 — Raw absolute values, capacity in MW ────────────────────────────────────────────
#
# Baseline. Capacity channel contains absolute installed capacity in MW.
# Weather channels are raw ERA5 values with no normalization.
# Model predicts MWh directly (output_capacity_factor=False).
...
# ── Option 3 — Spatial distribution capacity, z-score weather ─────────────────
#
...
# ── Option 4 — Spatial distribution capacity, raw absolute weather ─────────────────
#
...
# ── Option 5 — Spatiotemporal distribution capacity, z-score weather ───────────────
#
...
# ── Option 6 — Capacity-weighted ssrd only (Lindas et al.) ────────────────────
#
...
def task_build_solar_features_opt6(
        depends_on={
            "era5": ERA5_MASKED_SOLAR_DATA_DIR / ".complete",
            "capacity": SOLAR_CAPACITY_GRIDS_ZARR_STORE / ".complete",
            "smard": SMARD_SOLAR_GENERATION_SERIES_MASKED_CSV,
        ...
        produces=SOLAR_FEATURES_OPT6_DIR / ".complete",
):
    build_feature_dataset(
        **_COMMON,
        era5_variables=["ssrd"],   # t2m excluded — see docstring above
        out_dir=SOLAR_FEATURES_OPT6_DIR,
        capacity_as_spatial_distribution=False,
        capacity_as_spatiotemporal_distribution=False,
    ...
    (SOLAR_FEATURES_OPT6_DIR / ".complete").touch()
